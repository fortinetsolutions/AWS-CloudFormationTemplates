...
lb_info_line=$(aws ec2 describe-network-interfaces --filter "Name=subnet-id,Values=$Private1_SUBNET" --output text --query 'NetworkInterfaces[?contains(Description, `-gateway-lb`) == `true`].[PrivateIpAddress,NetworkInterfaceId,Description]')
...
lb_info_line=$(aws ec2 describe-network-interfaces --filter "Name=subnet-id,Values=$Private2_SUBNET" --output text --query 'NetworkInterfaces[?contains(Description, `-gateway-lb`) == `true`].[PrivateIpAddress,NetworkInterfaceId,Description]')
...
aws ec2 replace-route --route-table-id "$PrivateRouteTable2ID" --destination-cidr-block 0.0.0.0/0 --network-interface-id "$Fortigate1PrivateENI"
echo
